...
def process_videos_in_directory(input_directory, output_csv_path):
    ...
    # Get list of video files in the directory and subdirectories
    video_files = []
    ...
    if not video_files:
        print(f"No .mp4 video files found in {input_directory}")
        return
    else:
    ...
    with open(output_csv_path, mode='w', newline='') as csv_file:
        ...
        for video_file in tqdm(video_files, desc='Processing Videos', unit='video', ncols=80, ascii=True, file=sys.stdout):
            try:
                counters = process_video(video_file, face_mesh, mp_drawing)
                if counters is not None:
                    # Write results to CSV
                    writer.writerow({
                        'Video Name': os.path.basename(video_file),
...
def process_video(input_video_path, face_mesh, mp_drawing):
    ...
    # Use tqdm for frame progress bar
    frame_pbar = tqdm(total=total_frames, desc=f'Processing {os.path.basename(input_video_path)}', leave=False, unit='frame', ncols=80, ascii=True, file=sys.stdout)
...
if __name__ == "__main__":
    ...
    input_directory = r"C:\Users\idowe\Mind wandering research\data collection\Datasets\DAiSEE dataset\DAiSEE\DAiSEE\DataSet\Test"
    output_csv_path = r"C:\Users\idowe\Mind wandering research\data collection\Datasets\DAiSEE dataset\DAiSEE\DAiSEE\MWDetections\TestDetections\TestResults.csv"
